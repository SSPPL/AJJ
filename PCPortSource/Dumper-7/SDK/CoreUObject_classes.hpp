...
public:
	...
	// Hand-added by AJB-main (was in v28 CoreUObject_classes.hpp). Poke the GObjects
	// FUObjectItem internal flags with EInternalObjectFlags::RootSet (0x40000000) so the
	// object is kept alive by GC. v33 note: TUObjectArray::GetByIndex returns UObject*,
	// so we reach the FUObjectItem directly (Object* at +0x0, internal Flags at +0x8, UE4.20 layout).
	inline void AddToRootSet()
	{
		auto* Array = SDK::UObject::GObjects.GetTypedPtr();
		const int32 ChunkIndex = Index / SDK::TUObjectArray::ElementsPerChunk;
		const int32 InChunkIdx = Index % SDK::TUObjectArray::ElementsPerChunk;
		SDK::FUObjectItem* ChunkPtr = Array->GetDecrytedObjPtr()[ChunkIndex];
		if (!ChunkPtr) return;
		*reinterpret_cast<int32*>(reinterpret_cast<uint8*>(&ChunkPtr[InChunkIdx]) + 0x8) |= 0x40000000;
	}
...
{
public:
	uint8                                         Pad_1F8[0x1B8];                                    // 0x01F8(0x01B8)(Fixing Struct Size After Last Property [ Dumper-7 ])
